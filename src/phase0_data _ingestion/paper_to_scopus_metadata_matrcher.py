...
class MinerUMatcher:
    ...
    def __init__(self, papers_path: str, scopus_csv: str):
        ...
        self.mapping_file = self.papers_path / "paper_mapping.json"
        ...
        # Load data
        self.scopus_df = pd.read_csv(self.scopus_csv)
    ...
    def _create_metadata(self, scopus_row, method: str, confidence: float) -> dict:
        ...
        return {
            'eid': scopus_row['EID'],
            'doi': scopus_row.get('DOI'),
            'authors': scopus_row.get('Authors'),
            'year': int(scopus_row['Year']),
            'journal': scopus_row.get('Source title'),
            'abstract': scopus_row.get('Abstract'),
            'citations': int(scopus_row.get('Cited by', 0)),
            'match_method': method,
    ...
    def _save_results(self, results, stats):
        ...
        self.logger.info("")
        self.logger.info(f"✓ Enhanced: {self.mapping_file}")
