# -*- coding: utf-8 -*-
"""
Text cleaning for document preprocessing in GraphRAG pipeline.

Applies multi-stage cleaning to raw document text before chunking and extraction.
Handles encoding errors (mojibake), HTML artifacts from web scraping, LaTeX notation
from academic papers, markdown image placeholders, email addresses, and irrelevant
sections (acknowledgments, author info). Extracts reference sections separately for
provenance tracking while removing them from main content.

The cleaner uses clean-text library (wrapping ftfy) for encoding fixes and custom
regex patterns for format-specific cleaning. All operations are tracked with fix
counts for transparency. LaTeX cleaning preserves content while removing markup
(e.g., $1923^{40}$ → 1923, \textbf{AI} → AI). Section removal uses regex to match
markdown headings against configured garbage/reference section lists.

Examples:
    # Initialize with default sections from config
    from src.preprocessing.text_cleaner import TextCleaner

    cleaner = TextCleaner()

    # Clean scraped regulation text
    result = cleaner.clean(raw_text)
    print(f"Cleaned {result.original_length} → {result.cleaned_length} chars")
    print(f"Fixes: {result.fixes_applied}")  # {'encoding': 12, 'latex': 45, ...}
    print(f"Extracted {len(result.references)} references")

    # Clean without extracting references
    result = cleaner.clean(raw_text, extract_references=False)

    # Custom section lists
    cleaner = TextCleaner(
        garbage_sections=["ARTICLEINFO", "ACKNOWLEDGMENTS"],
        reference_sections=["References", "Bibliography"]
    )

References:
    clean-text: Text cleaning library with ftfy for encoding fixes
    config.extraction_config.PREPROCESSING_CONFIG: Default section lists
    CleaningResult dataclass: Structured cleaning output with statistics
"""
# Standard library
import re
import sys
