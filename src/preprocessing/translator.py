# -*- coding: utf-8 -*-
"""
Document translation for GraphRAG preprocessing pipeline.

Translates non-English documents to English using Google Cloud Translation API
with file-based caching to avoid redundant translation costs. Handles long texts
by splitting at sentence boundaries (5000 character limit per request). Provides
graceful degradation when API key is unavailable, logging untranslated documents
for manual review.

The translator maintains a cache index mapping (doc_id, source_lang) pairs to
cached translation files. Cache hits return immediately without API calls. For new
translations, text is chunked at sentence boundaries if longer than API limit,
translated chunk-by-chunk, and reassembled. All results include metadata (source
language, cache status, character counts) for cost tracking.

Examples:
    # Initialize with API key from environment
    from src.preprocessing.translator import Translator

    translator = Translator()  # Uses GOOGLE_TRANSLATE_API_KEY env var

    # Translate Spanish regulation
    result = translator.translate(
        text=spanish_text,
        source_lang="es",
        doc_id="ES_AI_Regulation"
    )
    print(f"Translated: {result.was_translated}")  # True
    print(f"From cache: {result.from_cache}")  # False (first time)
    print(f"Characters: {result.char_count}")

    # Subsequent call uses cache
    result2 = translator.translate(spanish_text, "es", "ES_AI_Regulation")
    print(f"From cache: {result2.from_cache}")  # True

    # Skip translation for English documents
    result3 = translator.translate(english_text, "en", "US_Regulation")
    print(f"Translated: {result3.was_translated}")  # False

    # Get cache statistics
    stats = translator.get_cache_stats()
    print(f"Cached: {stats['cached_documents']} docs")
    print(f"Languages: {stats['languages']}")
    print(f"Untranslated: {stats['untranslated_count']}")

References:
    Google Cloud Translation API: REST v2 endpoint with 5000 char/request limit
    TranslationResult dataclass: Structured translation output with metadata
    config.extraction_config.PREPROCESSING_CONFIG: Preprocessing settings
    data/interim/translation_cache/: Cache directory with cache_index.json
"""
# Standard library
import os
import json
